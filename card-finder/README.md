...
3. **Choose who to ask**: everyone holding the card is listed and ticked by default. Send, and each gets a Slack DM from the Card Finder app (a preview when Slack isn't connected).
   ...
   After sending, the success screen shows all purposes with the ones you've asked for before highlighted, and asks: *Do you want to fetch your credit report for credit card recommendations for the above purposes?* (a soft pull that doesn't affect the score). **Later** closes it. **Yes, Fetch Credit Report** shows a 3-second loading step, then 3 recommended cards; picking one opens it with *Find a referral* expanded.
